...
export function attachAttacks(enemyData){
  ...
  if(!sprite) return;
  try {
    const attacks = buildAttacksFor(enemyData);
    // ─── Normalisation: force attack.id à correspondre à la clé du dict ───
    // Sinon le bestiaire utilise attack.id comme data-attack du bouton, et
    // detailSprite.attacks[clickedId] retourne undefined si la clé du dict
    // est différente. Cause de "écran noir au clic" sans message d'erreur.
    if(attacks){
      for(const key of Object.keys(attacks)){
        if(attacks[key] && attacks[key].id !== key){
          attacks[key].id = key;
        }
      }
    }
    sprite.attacks = attacks;
  } catch(e){
